...
class Payu extends CI_Controller {
	...
	{	
		...
		// udf1 - organization id
		// udf2 - plan id
		// To check empty field or not
		if($data['udf1']!='' && $data['udf2']!='' && ($data['plan_option']=="original" || $data['plan_option']=="upgrade" || $data['plan_option']=="renewal")) {
			...
			// Upgrade Plan
				...
				else if(empty($input['organization_current_plan']) || (isset($input['organization_current_plan']['subscription_id']) && $input['organization_current_plan']['subscription_id']!=$data['udf2'])) {
		...
		if(isset($proper) && $proper == 1 && $data['amount'] > 0) {
			...
			$valid = $this->job_provider_model->orginal_plan_valid($data['udf2'],$data['udf1']);
			if($valid == 2) {
				$subscription_plan_data = $this->common_model->subcription_plan($data['udf2']);
				$no_of_days = $subscription_plan_data[0]['subscription_validity_days'] - 1;
				$user_subscription_data = array(
												'organization_id' 						=> $data['udf1'],
												'subscription_id' 					    => $data['udf2'],
				...
				if($this->job_provider_model->insert_orignial_plan($user_subscription_data)) {
					$email_data['subscription_details'] = $this->common_model->provider_subscription_active_plans($data['udf1']);
					// Email configuration
